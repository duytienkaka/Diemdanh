        ...
        <?php
            ...
            if ($effectiveStatus !== 'ended') {
                if (
                    ...
                    $selectedSession['start_time']   <= $time &&
                    $selectedSession['end_time']     >  $time
                ) {
                    // Đang trong khoảng giờ học
                    $effectiveStatus = 'ongoing';
                } elseif (
                    $selectedSession['session_date'] < $today ||
                    ($selectedSession['session_date'] === $today && $selectedSession['end_time'] <= $time)
                ) {
                    // Đã qua buổi học
        ...
        <div class="card" style="margin-bottom: 24px;">
            <div style="padding: 20px;">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px;">
                    <div>
                        <div style="font-size: 11px; color: #6b7280; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;"><i class="fas fa-book"></i> Môn học</div>
                        <div style="font-size: 14px; font-weight: 600; color: #1f2937;"><?= htmlspecialchars($selectedSession['subject_code']) ?> - <?= htmlspecialchars($selectedSession['subject_name']) ?></div>
                    </div>
                    <div>
                        <div style="font-size: 11px; color: #6b7280; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;"><i class="fas fa-calendar"></i> Ngày</div>
                        <div style="font-size: 14px; font-weight: 600; color: #1f2937;"><?= date('d/m/Y', strtotime($selectedSession['session_date'])) ?></div>
                    </div>
                    <div>
                        <div style="font-size: 11px; color: #6b7280; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;"><i class="fas fa-clock"></i> Giờ</div>
                        <div style="font-size: 14px; font-weight: 600; color: #1f2937;"><?= htmlspecialchars($selectedSession['start_time']) ?> - <?= htmlspecialchars($selectedSession['end_time']) ?></div>
                    </div>
                    <div>
                        <div style="font-size: 11px; color: #6b7280; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;"><i class="fas fa-layer-group"></i> Học kỳ</div>
                        <div style="font-size: 14px; font-weight: 600; color: #1f2937;"><?= htmlspecialchars($selectedSession['semester_name']) ?></div>
                    </div>
                    <div>
                        <div style="font-size: 11px; color: #6b7280; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;"><i class="fas fa-tag"></i>Trạng thái</div>
                        <div>
                    ...
                    <div>
                        <div style="font-size: 11px; color: #6b7280; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;"><i class="fas fa-check-double"></i> Điểm danh</div>
                        <div>
